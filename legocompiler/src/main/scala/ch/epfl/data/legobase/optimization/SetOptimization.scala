...
class SetLinkedListTransformation(override val IR: SetComponent with pardis.deep.scalalib.ArrayComponent with pardis.deep.scalalib.IntComponent with pardis.deep.scalalib.BooleanComponent with ContOps with ManualLiftedLegoBase) extends pardis.optimization.RecursiveRuleBasedTransformer[SetComponent with pardis.deep.scalalib.ArrayComponent with pardis.deep.scalalib.IntComponent with pardis.deep.scalalib.BooleanComponent with ContOps with ManualLiftedLegoBase](IR) {
  import IR._
  type Rep[T] = IR.Rep[T]
  type Var[T] = IR.Var[T]
  ...
  def shouldBeLowered[T](sym: Rep[T]): Boolean = loweredSets.contains(sym.asInstanceOf[Rep[Any]])
  ...
  rewrite += rule {
    case node @ SetRetain(nodeself, nodep) if shouldBeLowered(nodeself) =>
      ...
      {
        var prev: this.Var[ch.epfl.data.pardis.shallow.scalalib.collection.Cont[A]] = __newVar(unit(null));
        var current: this.Var[ch.epfl.data.pardis.shallow.scalalib.collection.Cont[A]] = __newVar(self.headCont);
        __whileDo(infix_$bang$eq(__readVar(current), unit(null)), {
  ...
  rewrite += rule {
    case node @ SetFoldLeft(nodeself, nodez, nodeop) if shouldBeLowered(nodeself) =>
      ...
      implicit val typeA = transformType(nodeself.tp.typeArguments(0)).asInstanceOf[TypeRep[A]]
      implicit val typeB = transformType(z.tp.typeArguments(0)).asInstanceOf[TypeRep[B]]
