...
//   case class HashJoinOpForeach[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[Unit](Some(self), "foreach", List(List(f))) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOpFindFirst[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "findFirst", List(List(cond))) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOpNullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C, D](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val typeD: TypeRep[D], val di: DummyImplicit) extends FunctionDef[D](Some(self), "NullDynamicRecord", List()) {
...
//   case class HashJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "next", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class HashJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class HashJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[B])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[B](self, "tmpLine", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOp_Field_TmpLine[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[B](self, "tmpLine") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class HashJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[A]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[ArrayBuffer[A]](self, "tmpBuffer", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOp_Field_TmpBuffer[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[ArrayBuffer[A]](self, "tmpBuffer") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class HashJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[Int])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[Int](self, "tmpCount", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOp_Field_TmpCount[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[Int](self, "tmpCount") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class HashJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[DynamicCompositeRecord[A, B]](self, "NullDynamicRecord") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true
...
//   def hashJoinOpForeach[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOpForeach[A, B, C](self, f)
//   def hashJoinOpFindFirst[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = HashJoinOpFindFirst[A, B, C](self, cond)
//   def hashJoinOpNullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C, D](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], typeD: TypeRep[D], di: DummyImplicit): Rep[D] = HashJoinOpNullDynamicRecord[A, B, C, D](self)
...
//   def hashJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = HashJoinOpNext[A, B, C](self)
//   def hashJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOpClose[A, B, C](self)
...
//   def hashJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[B])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOp_Field_TmpLine_$eq[A, B, C](self, x$1)
//   def hashJoinOp_Field_TmpLine[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[B] = HashJoinOp_Field_TmpLine[A, B, C](self)
...
//   def hashJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[A]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOp_Field_TmpBuffer_$eq[A, B, C](self, x$1)
//   def hashJoinOp_Field_TmpBuffer[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[ArrayBuffer[A]] = HashJoinOp_Field_TmpBuffer[A, B, C](self)
//   def hashJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[Int])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOp_Field_TmpCount_$eq[A, B, C](self, x$1)
//   def hashJoinOp_Field_TmpCount[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Int] = HashJoinOp_Field_TmpCount[A, B, C](self)
...
//   def hashJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = HashJoinOp_Field_NullDynamicRecord[A, B, C](self)
//   type HashJoinOp[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C] = ch.epfl.data.legobase.queryengine.volcano.HashJoinOp[A, B, C]
...
//     }
//   }
//   override def hashJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = {
//     {
...
//     }
//   }
//   override def hashJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = {
//     unit(())
//   }
...
//   case class NestedLoopsJoinOpForeach[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[Unit](Some(self), "foreach", List(List(f))) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOpFindFirst[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "findFirst", List(List(cond))) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOpNullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, D](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeD: TypeRep[D], val di: DummyImplicit) extends FunctionDef[D](Some(self), "NullDynamicRecord", List()) {
//     override def curriedConstructor = (copy[A, B, D] _)
//   }
...
//   case class NestedLoopsJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "next", List(List())) {
//     override def curriedConstructor = (copy[A, B] _)
//   }

//   case class NestedLoopsJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
//     override def curriedConstructor = (copy[A, B] _)
//   }
...
//   case class NestedLoopsJoinOp_Field_Cnt_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[Int])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldSetter[Int](self, "cnt", x$1) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOp_Field_Cnt[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldGetter[Int](self, "cnt") {
//     override def curriedConstructor = (copy[A, B] _)
//   }

//   case class NestedLoopsJoinOp_Field_RightTuple_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[B])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldSetter[B](self, "rightTuple", x$1) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOp_Field_RightTuple[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldGetter[B](self, "rightTuple") {
//     override def curriedConstructor = (copy[A, B] _)
//   }
...
//   case class NestedLoopsJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldDef[DynamicCompositeRecord[A, B]](self, "NullDynamicRecord") {
//     override def curriedConstructor = (copy[A, B] _)
//     override def isPure = true
...
//   def nestedLoopsJoinOpForeach[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOpForeach[A, B](self, f)
//   def nestedLoopsJoinOpFindFirst[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = NestedLoopsJoinOpFindFirst[A, B](self, cond)
//   def nestedLoopsJoinOpNullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, D](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeD: TypeRep[D], di: DummyImplicit): Rep[D] = NestedLoopsJoinOpNullDynamicRecord[A, B, D](self)
...
//   def nestedLoopsJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = NestedLoopsJoinOpNext[A, B](self)
//   def nestedLoopsJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOpClose[A, B](self)
...
//   def nestedLoopsJoinOp_Field_Cnt_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[Int])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOp_Field_Cnt_$eq[A, B](self, x$1)
//   def nestedLoopsJoinOp_Field_Cnt[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Int] = NestedLoopsJoinOp_Field_Cnt[A, B](self)
//   def nestedLoopsJoinOp_Field_RightTuple_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[B])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOp_Field_RightTuple_$eq[A, B](self, x$1)
//   def nestedLoopsJoinOp_Field_RightTuple[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[B] = NestedLoopsJoinOp_Field_RightTuple[A, B](self)
...
//   def nestedLoopsJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = NestedLoopsJoinOp_Field_NullDynamicRecord[A, B](self)
//   type NestedLoopsJoinOp[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record] = ch.epfl.data.legobase.queryengine.volcano.NestedLoopsJoinOp[A, B]
...
//     }
//   }
//   override def nestedLoopsJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = {
//     {
//       var exit: this.Var[Boolean] = __newVar(unit(false));
...
//     }
//   }
//   override def nestedLoopsJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = {
//     unit(())
//   }
...
//   case class LeftOuterJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val evidence$1: Manifest[B]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class LeftOuterJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[A])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[A](self, "tmpLine", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOp_Field_TmpLine[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[A](self, "tmpLine") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class LeftOuterJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[ArrayBuffer[B]](self, "tmpBuffer", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOp_Field_TmpBuffer[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[ArrayBuffer[B]](self, "tmpBuffer") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class LeftOuterJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[Int])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[Int](self, "tmpCount", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOp_Field_TmpCount[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[Int](self, "tmpCount") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class LeftOuterJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[DynamicCompositeRecord[A, B]](self, "NullDynamicRecord") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true
...
//   def leftOuterJoinOpForeach[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = LeftOuterJoinOpForeach[A, B, C](self, f)
//   def leftOuterJoinOpFindFirst[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[DynamicCompositeRecord[A, B]] = LeftOuterJoinOpFindFirst[A, B, C](self, cond)
...
//   def leftOuterJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[DynamicCompositeRecord[A, B]] = LeftOuterJoinOpNext[A, B, C](self)
//   def leftOuterJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = LeftOuterJoinOpClose[A, B, C](self)
...
//   def leftOuterJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[A])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = LeftOuterJoinOp_Field_TmpLine_$eq[A, B, C](self, x$1)
//   def leftOuterJoinOp_Field_TmpLine[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[A] = LeftOuterJoinOp_Field_TmpLine[A, B, C](self)
//   def leftOuterJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = LeftOuterJoinOp_Field_TmpBuffer_$eq[A, B, C](self, x$1)
//   def leftOuterJoinOp_Field_TmpBuffer[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[ArrayBuffer[B]] = LeftOuterJoinOp_Field_TmpBuffer[A, B, C](self)
//   def leftOuterJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[Int])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = LeftOuterJoinOp_Field_TmpCount_$eq[A, B, C](self, x$1)
//   def leftOuterJoinOp_Field_TmpCount[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Int] = LeftOuterJoinOp_Field_TmpCount[A, B, C](self)
...
//   def leftOuterJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = LeftOuterJoinOp_Field_NullDynamicRecord[A, B, C](self)
//   type LeftOuterJoinOp[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C] = ch.epfl.data.legobase.queryengine.volcano.LeftOuterJoinOp[A, B, C]
...
//     }
//   }
//   override def leftOuterJoinOpNext[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[DynamicCompositeRecord[A, B]] = {
//     {
...
//     }
//   }
//   override def leftOuterJoinOpClose[A <: ch.epfl.data.pardis.shallow.Record, B <: ch.epfl.data.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = {
//     unit(())
//   }
