...
trait AggOpOps extends Base { this: OperatorsComponent =>
  ...
  // case classes
  case class AggOpNew[A, B](parent: Rep[Operator[A]], numAggs: Rep[Int], grpInput1: Sym[A], grpOutput: Block[B], aggFuncs: ((Rep[A], Rep[Double]) => Rep[Double])*)(implicit manifestA: Manifest[A], manifestB: Manifest[B]) extends FunctionDef[AggOp[A, B]](None, "new AggOp", List(List(parent, numAggs), List(Lambda(grpInput1, grpOutput)), List(aggFuncs.map(x => doLambda2(x)): _*)))
  ...
  def aggOpNew[A, B](parent: Rep[Operator[A]], numAggs: Rep[Int], grp: ((Rep[A]) => Rep[B]), aggFuncs: ((Rep[A], Rep[Double]) => Rep[Double])*)(implicit manifestA: Manifest[A], manifestB: Manifest[B]): Rep[AggOp[A, B]] = {
    ...
    AggOpNew[A, B](parent, numAggs, grpInput1, grpOutput, aggFuncs: _*)
  }
...
trait MapOpOps extends Base { this: OperatorsComponent =>
  ...
  // case classes
  case class MapOpNew[A](parent: Rep[Operator[A]], aggFuncs: ((Rep[A]) => Rep[Unit])*)(implicit manifestA: Manifest[A]) extends FunctionDef[MapOp[A]](None, "new MapOp", List(List(parent), List(aggFuncs.map(x => doLambda(x)): _*)))
  ...
  // method definitions
  def mapOpNew[A](parent: Rep[Operator[A]], aggFuncs: ((Rep[A]) => Rep[Unit])*)(implicit manifestA: Manifest[A]): Rep[MapOp[A]] = MapOpNew[A](parent, aggFuncs: _*)
