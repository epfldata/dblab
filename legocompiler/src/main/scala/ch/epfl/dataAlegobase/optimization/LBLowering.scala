...
class LBLowering(override val from: LoweringLegoBase, override val to: LoweringLegoBase, override val generateHashAndEqual: Boolean) extends Lowering[LoweringLegoBase, LoweringLegoBase](from, to) {
  ...
  // def ==(o: Any) = false
  def getEqEq(tpe: TypeRep[Any]): Option[PardisStructMethod] = {
    if (generateHashAndEqual) {
      val lam = doLambda2Def((x: Rep[Any], y: Rep[Any]) => {
        unit(false)
      })(tpe, AnyType, BooleanType).asInstanceOf[PardisLambdaDef]
      Some(PardisStructMethod("equals", lam, true))
    } else
      None
  }
  ...
  override def transformDef[T: TypeRep](node: Def[T]): to.Def[T] = node match {
    ...
    case ps @ PardisStruct(tag, elems, methods) =>
      ...
      val newMethods = List(PardisStructMethod("equal", getEquals(newTpe, newFields)),
        PardisStructMethod("hash", getHash(newTpe, newFields))) ++ getEqEq(newTpe) ++ getPrint(newTpe, newFields)
    ...
    case ConcatDynamic(record1, record2, leftAlias, rightAlias) if lowerStructs => {
      ...
      val methods = {
        ...
        List(PardisStructMethod("equal", eqMethod), PardisStructMethod("hash", hashMethod)) ++ getEqEq(newTpe.asInstanceOf[TypeRep[Any]]) ++ printMethod
      }
