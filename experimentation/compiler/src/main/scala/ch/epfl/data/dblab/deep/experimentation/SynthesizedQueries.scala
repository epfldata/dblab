  ...
  def Q12Synthesized(numRuns: Rep[Int], fields: Int): Rep[Unit] = {
    {
      ...
      intWrapper(unit(0)).until(numRuns).foreach[Unit](__lambda(((i: this.Rep[Int]) => GenericEngine.runQuery[Unit]({
        ...
        val so2 = __newSelectOp(__newScanOp(lineitemTable))(__lambda(((x: this.Rep[ch.epfl.data.dblab.experimentation.tpch.LINEITEMRecord]) => {
        ...
        // val aggOp: this.Rep[ch.epfl.data.dblab.queryengine.push.AggOp[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[ch.epfl.data.dblab.experimentation.tpch.ORDERSRecord, ch.epfl.data.dblab.experimentation.tpch.LINEITEMRecord], OptimalString]] = __newAggOp(jo, unit(2))(__lambda(((x: this.Rep[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[ch.epfl.data.dblab.experimentation.tpch.ORDERSRecord, ch.epfl.data.dblab.experimentation.tpch.LINEITEMRecord]]) => x.selectDynamic[OptimalString](unit("L_SHIPMODE")))))(__lambda(((t: this.Rep[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[ch.epfl.data.dblab.experimentation.tpch.ORDERSRecord, ch.epfl.data.dblab.experimentation.tpch.LINEITEMRecord]], currAgg: this.Rep[Double]) => __ifThenElse(t.selectDynamic[OptimalString](unit("O_ORDERPRIORITY")).$eq$eq$eq(URGENT).$bar$bar(t.selectDynamic[OptimalString](unit("O_ORDERPRIORITY")).$eq$eq$eq(HIGH)), currAgg.$plus(unit(1)), currAgg))), __lambda(((t: this.Rep[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[ch.epfl.data.dblab.experimentation.tpch.ORDERSRecord, ch.epfl.data.dblab.experimentation.tpch.LINEITEMRecord]], currAgg: this.Rep[Double]) => __ifThenElse(t.selectDynamic[OptimalString](unit("O_ORDERPRIORITY")).$eq$bang$eq(URGENT).$amp$amp(t.selectDynamic[OptimalString](unit("O_ORDERPRIORITY")).$eq$bang$eq(HIGH)), currAgg.$plus(unit(1)), currAgg))));
