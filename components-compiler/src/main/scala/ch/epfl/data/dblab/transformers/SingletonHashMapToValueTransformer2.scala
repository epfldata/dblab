...
import QueryEngineIR.Sqd.Predef._
import QueryEngineIR.Sqd.Quasicodes._
...
object SingletonHashMapToValueTransformer2 extends SimpleRuleBasedTransformer with TopDownTransformer with QueryEngineIR.Sqd.SelfTransformer with TransformerHandler {
  ...
  rewrite {
    ...
    case ir"val $hm: HashMap[$kt,$vt] = $init; $body: $bt" =>
      ...
      //var uniqueUse = Option.empty[(kt.Typ, IR[vt.Typ, init.Ctx])]
      ...
      val body2 = body rewrite {
        ...
        case ir"$$hm.getOrElseUpdate(${ key }, $value)" if uniqueUse.isEmpty && key.rep.isInstanceOf[Constant[_]] =>
          ...
          //uniqueUse = Some(key -> closedValue)
          //ir"$value"
          ir"$$value: $vt"
      ...
      //ir"val value = ${uniqueUse map (_._2) getOrElse giveUp}; $body3"
