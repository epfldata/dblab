 ...
 */
class MemoryAllocationHoist(override val IR: QueryEngineExp, val schema: Schema)
  ...
  def createBuffers() {
    ...
    for (mallocInstance <- distinctInstances) {
      ...
      val poolSize = unit(getPoolSize(mallocInstance))
  ...
  rewrite += rule {
    case ps @ ArrayNew(size) if startCollecting => {
      ...
      __assign(bufferInfo.index, readVar(bufferInfo.index)(IntType) + unit(1))
      p
    }
  }
