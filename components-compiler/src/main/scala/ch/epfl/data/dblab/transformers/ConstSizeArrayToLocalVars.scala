...
object ConstSizeArrayToLocalVars extends TransformerHandler {
  def apply[Lang <: Base, T: PardisType](context: Lang)(block: context.Block[T]): context.Block[T] = {
    new ConstSizeArrayToLocalVars(context.asInstanceOf[QueryEngineExp]).optimize(block)
  }
}
 ...
 */
class ConstSizeArrayToLocalVars(override val IR: QueryEngineExp) extends RecursiveRuleBasedTransformer[QueryEngineExp](IR)
