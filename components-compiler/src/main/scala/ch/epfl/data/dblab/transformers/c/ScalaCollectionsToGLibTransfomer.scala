 ...
 */
class ScalaCollectionsToGLibTransfomer(override val IR: QueryEngineExp) extends RecursiveRuleBasedTransformer[QueryEngineExp](IR) with CTransformer {
  import IR._
  import CNodes._
  import CTypes._
  ...
  rewrite += rule {
    case abn @ (ArrayBufferNew2()) =>
      ...
      g_array_new(unit(0), unit(1), sizeof[X])
  }
  rewrite += rule {
  ...
  rewrite += rule {
    case ArrayBufferAppend(a, e) =>
      g_array_append_vals(apply(a).asInstanceOf[Rep[LPointer[GArray]]], CLang.&(e.asInstanceOf[Rep[gconstpointer]]), unit(1))
  }
  rewrite += rule {
