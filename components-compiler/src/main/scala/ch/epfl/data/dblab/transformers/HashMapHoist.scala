...
object HashMapHoist extends TransformerHandler {
  def apply[Lang <: Base, T: PardisType](context: Lang)(block: context.Block[T]): context.Block[T] = {
    new HashMapHoist(context.asInstanceOf[QueryEngineExp]).optimize(block)
  }
}
 ...
 */
class HashMapHoist(override val IR: QueryEngineExp) extends Optimizer[QueryEngineExp](IR) {
  import IR._
  import CNodes._
  import CTypes._
