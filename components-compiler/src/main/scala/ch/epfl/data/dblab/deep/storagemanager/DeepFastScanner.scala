...
trait FastScannerOps extends Base with OptimalStringOps {
  ...
  implicit class FastScannerRep(self: Rep[FastScanner]) {
    ...
    def close(): Rep[Unit] = fastScannerClose(self)
  ...
  val FastScannerClose = FastScannerIRs.FastScannerClose
  type FastScannerClose = FastScannerIRs.FastScannerClose
  ...
  def fastScannerClose(self: Rep[FastScanner]): Rep[Unit] = FastScannerClose(self)
...
object FastScannerIRs extends Base {
  ...
  case class FastScannerClose(self: Rep[FastScanner]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
    override def curriedConstructor = (copy _)
  }
...
object FastScannerQuasiNodes extends BaseExtIR {
  ...
  case class FastScannerCloseExt(self: Rep[FastScanner]) extends FunctionDef[FastScannerClose, Unit] {
    override def nodeUnapply(t: FastScannerClose): Option[Product] = (FastScannerClose.unapply(t): Option[Product]) map { r =>
      r
    }
  }
...
trait FastScannerExtOps extends BaseExt with OptimalStringExtOps {
  ...
  def fastScannerClose(self: Rep[FastScanner]): Rep[Unit] = FastScannerCloseExt(self)
