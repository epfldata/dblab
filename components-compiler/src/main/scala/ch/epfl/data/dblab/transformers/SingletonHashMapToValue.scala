...
object SingletonHashMapToValueTransformer extends TransformerHandler {
  def apply[Lang <: Base, T: PardisType](context: Lang)(block: context.Block[T]): context.Block[T] = {
    new SingletonHashMapToValueTransformer(context.asInstanceOf[QueryEngineExp]).optimize(block)
  }
}
 ...
 */
class SingletonHashMapToValueTransformer(override val IR: QueryEngineExp)
  extends RecursiveRuleBasedTransformer[QueryEngineExp](IR) {
  import IR._
