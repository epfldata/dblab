 ...
 */
class QueryMonadUnfoldLowering(val schema: Schema, override val IR: QueryEngineExp, val QML: QueryMonadLowering) extends RuleBasedTransformer[QueryEngineExp](IR) with StructProcessing[QueryEngineExp] {
  import IR._
  ...
  abstract class QueryUnfold[T: TypeRep] { self =>
    ...
    def groupByMapValues[K: TypeRep, S: TypeRep](par: Rep[T => K], pred: Option[Rep[T => Boolean]])(func: Rep[Array[T] => S]): QueryUnfold[(K, S)] = new QueryUnfold[(K, S)] {
      type V = T
      type Source = Int
      def sourceType: TypeRep[Source] = typeRep[Int]
      ...
      def next(s: Rep[Source]): Rep[((K, S), Source)] = {
        ...
        iteratorMap += input -> QueryUnfold(arr).asInstanceOf[QueryUnfold[Any]]
  ...
  implicit def queryToUnfold[T](sym: Rep[Query[T]]): QueryUnfold[T] = {
    // System.out.println(s"finding cps for $sym")
    val cps = iteratorMap(sym.asInstanceOf[Rep[Any]]).asInstanceOf[QueryUnfold[T]]
    // System.out.println(s"tp associated to sym $sym is: ${cps.tp}")
    cps
  }

  val iteratorMap = mutable.Map[Rep[Any], QueryUnfold[Any]]()

  rewrite += statement {
    case sym -> QueryNew2(array) =>
      val cps = QueryUnfold(array)(array.tp.typeArguments(0).asInstanceOf[TypeRep[Any]])
      iteratorMap += sym -> cps
      sym
  }
  ...
  rewrite += statement {
    case sym -> QueryFilter(monad, p) =>
      val cps = queryToUnfold(monad).filter(p)
      iteratorMap += sym -> cps
      sym
  }
  ...
  rewrite += statement {
    case sym -> QueryMap(monad, f) =>
      val cps = queryToUnfold(monad).map(f)(f.tp.typeArguments(1).asInstanceOf[TypeRep[Any]])
      iteratorMap += sym -> cps
      sym
  }
  ...
  rewrite += statement {
    case sym -> QueryForeach(monad, f) =>
      // val cps = queryToUnfold(monad).foreach(i => inlineFunction(f, i)).asInstanceOf[QueryUnfold[Any]]
      // iteratorMap += sym -> cps
      // sym
      queryToUnfold(monad).foreach(i => inlineFunction(f, i))
  }
  ...
  //     queryToUnfold(monad).minBy(by)(by.tp.typeArguments(1).asInstanceOf[TypeRep[Any]])
  // }

  rewrite += rule {
    case QueryCount(monad) =>
      queryToUnfold(monad).count
  }

  // rewrite += rule {
  //   case QueryAvg(monad) =>
  //     queryToUnfold(monad).avg
  // }

  rewrite += rule {
    case QuerySum(monad) =>
      queryToUnfold(monad).sum
  }

  // rewrite += statement {
  //   case sym -> QueryTake(monad, num) =>
  //     val cps = queryToUnfold(monad).take(num)
  //     iteratorMap += sym -> cps
  //     sym
  // }
  ...
  rewrite += statement {
    case sym -> JoinableQueryLeftHashSemiJoin(monad1, monad2, leftHash, rightHash, joinCond) => {
      ...
      iteratorMap += sym -> cps.asInstanceOf[QueryUnfold[Any]]
      sym
    }
  }
