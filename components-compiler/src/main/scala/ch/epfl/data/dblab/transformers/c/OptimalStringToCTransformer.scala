 ...
 */
class OptimalStringToCTransformer(override val IR: QueryEngineExp) extends RecursiveRuleBasedTransformer[QueryEngineExp](IR) with CTransformer {
  import IR._
  import CNodes._
  import CTypes._
  import sc.cscala.CLangTypes
  ...
  rewrite += rule {
    case OptimalStringEndsWith(self, y) =>
      {
        val lenx: Rep[ch.epfl.data.sc.cscala.CLangTypes.CSize] = CString.strlen(new OptimalStringRep(self).getBaseValue(self));
        val leny: Rep[ch.epfl.data.sc.cscala.CLangTypes.CSize] = CString.strlen(new OptimalStringRep(self).getBaseValue(y));
  ...
  rewrite += rule {
    case OptimalStringIndexOfSlice(self, y, idx) =>
      {
        val substr: Rep[ch.epfl.data.sc.cscala.CLangTypes.LPointer[Char]] = CString.strstr(CLang.pointer_add[Char](new OptimalStringRep(self).getBaseValue(self), idx)(typeRep[Char], CLangTypes.charType), new OptimalStringRep(self).getBaseValue(y));
    ...
    case OptimalStringSlice(self, start, end) =>
      {
        ...
        val newbuf: Rep[ch.epfl.data.sc.cscala.CLangTypes.LPointer[Char]] = CStdLib.malloc[Char](len);
