 ...
 */
class ScalaScannerToCFScanfTransformer(override val IR: QueryEngineExp) extends RuleBasedTransformer[QueryEngineExp](IR) {
  import IR._
  import CNodes._
  import CTypes._
  ...
  override def transformType[T: PardisType]: PardisType[Any] = ({
    val tp = typeRep[T]
    if (tp == typeFastScanner) typePointer(typeFILE)
    else super.transformType[T]
  }).asInstanceOf[PardisType[Any]]

  rewrite += rule {
    case FastScannerNew(f) => CStdIO.fopen(f.asInstanceOf[Rep[LPointer[Char]]], unit("r"))
  }
  rewrite += rule {
    case FastScannerNext_int(s) =>
      val v = readVar(__newVar[Int](unit(0)))
  ...
  rewrite += rule { case FastScannerHasNext(s) => unit(true) }
  rewrite += rule {
