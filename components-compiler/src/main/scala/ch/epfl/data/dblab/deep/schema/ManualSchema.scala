...
trait SchemaOps extends Base with TableOps {
  type Schema = ch.epfl.data.dblab.schema.Schema
...
object SchemaIRs extends Base {
  case object SchemaType extends TypeRep[ch.epfl.data.dblab.schema.Schema] {
    def rebuild(newArguments: TypeRep[_]*): TypeRep[_] = SchemaType
    val name = "Schema"
    val typeArguments = Nil
    val typeTag = scala.reflect.runtime.universe.typeTag[ch.epfl.data.dblab.schema.Schema]
  }
  implicit val typeSchema: TypeRep[ch.epfl.data.dblab.schema.Schema] = SchemaType
  type Schema = ch.epfl.data.dblab.schema.Schema
}
