...
// Manual changes required:
//  - change new Rep[Cont](...) to __newCont(...)
//  - fix constructor
...
@reflect[scala.collection.mutable.Set[_]]
@transformation
class SetLinkedList[T](var listHead: Cont[T]) {
  def this() = this(null)

  def +=(elem: T) = {
    listHead = Cont(elem, listHead)
  }
  ...
  def retain(p: T => Boolean): Unit = {
    ...
    while (current != null) {
      //  if (p(current.elem)) {
      //    if (prev == null) listHead = current.next
      ...
      if (!p(current.elem)) {
        if (listHead == current) listHead = null
        if (prev != null) prev.next = null
      } else {
        if (prev != null) prev.next = current
        if (listHead == null) listHead = current
        prev = current
      }
      current = current.next
    }
...
class SetArray[T: Manifest] {
  ...
  val array: Array[T] = new Array[T](5)
