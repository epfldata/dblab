...
// import sc.pardis.ir.pardisTypeImplicits._
...
//   type Operator[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.Operator[A]
...
//         __ifThenElse(infix_$eq$eq(readVar(res), self.NullDynamicRecord), __assign(exit, unit(true)), __assign(exit, __app(cond).apply(readVar(res))))
//       });
//       readVar(res)
//     }
//   }
...
//   type ScanOp[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.ScanOp[A]
...
//   type SelectOp[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.SelectOp[A]
...
//   type AggOp[A, B] = ch.epfl.data.dblab.legobase.queryengine.volcano.AggOp[A, B]
...
//           aggs.update(readVar(i), __app(aggFun).apply(t, aggs.apply(readVar(i))));
...
//   type SortOp[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.SortOp[A]
...
//   type MapOp[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.MapOp[A]
...
//   type PrintOp[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.PrintOp[A]
...
//   case class HashJoinOpForeach[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[Unit](Some(self), "foreach", List(List(f))) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOpFindFirst[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "findFirst", List(List(cond))) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOpNullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C, D](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val typeD: TypeRep[D], val di: DummyImplicit) extends FunctionDef[D](Some(self), "NullDynamicRecord", List()) {
...
//   case class HashJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "next", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class HashJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class HashJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[B])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[B](self, "tmpLine", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOp_Field_TmpLine[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[B](self, "tmpLine") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class HashJoinOp_Field_Hm[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[HashMap[C, ArrayBuffer[A]]](self, "hm") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true

//   }

//   case class HashJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[A]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[ArrayBuffer[A]](self, "tmpBuffer", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOp_Field_TmpBuffer[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[ArrayBuffer[A]](self, "tmpBuffer") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class HashJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[Int])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[Int](self, "tmpCount", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class HashJoinOp_Field_TmpCount[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[Int](self, "tmpCount") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class HashJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[DynamicCompositeRecord[A, B]](self, "NullDynamicRecord") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true
...
//   def hashJoinOpForeach[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOpForeach[A, B, C](self, f)
//   def hashJoinOpFindFirst[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = HashJoinOpFindFirst[A, B, C](self, cond)
//   def hashJoinOpNullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C, D](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], typeD: TypeRep[D], di: DummyImplicit): Rep[D] = HashJoinOpNullDynamicRecord[A, B, C, D](self)
...
//   def hashJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = HashJoinOpNext[A, B, C](self)
//   def hashJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOpClose[A, B, C](self)
...
//   def hashJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[B])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOp_Field_TmpLine_$eq[A, B, C](self, x$1)
//   def hashJoinOp_Field_TmpLine[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[B] = HashJoinOp_Field_TmpLine[A, B, C](self)
//   def hashJoinOp_Field_Hm[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[HashMap[C, ArrayBuffer[A]]] = HashJoinOp_Field_Hm[A, B, C](self)
//   def hashJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[A]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOp_Field_TmpBuffer_$eq[A, B, C](self, x$1)
//   def hashJoinOp_Field_TmpBuffer[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[ArrayBuffer[A]] = HashJoinOp_Field_TmpBuffer[A, B, C](self)
//   def hashJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]], x$1: Rep[Int])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = HashJoinOp_Field_TmpCount_$eq[A, B, C](self, x$1)
//   def hashJoinOp_Field_TmpCount[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Int] = HashJoinOp_Field_TmpCount[A, B, C](self)
...
//   def hashJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = HashJoinOp_Field_NullDynamicRecord[A, B, C](self)
//   type HashJoinOp[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C] = ch.epfl.data.dblab.legobase.queryengine.volcano.HashJoinOp[A, B, C]
...
//   }
//   implicit def typeHashJoinOp[A <: ch.epfl.data.sc.pardis.shallow.Record: TypeRep, B <: ch.epfl.data.sc.pardis.shallow.Record: TypeRep, C: TypeRep] = HashJoinOpType(implicitly[TypeRep[A]], implicitly[TypeRep[B]], implicitly[TypeRep[C]])
// }
...
//     }
//   }
//   override def hashJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = {
//     {
...
//           __whileDo(readVar(found).unary_$bang.$amp$amp(readVar(i).$less(self.tmpBuffer.size)), __ifThenElse(__app(self.joinCond).apply(self.tmpBuffer.apply(readVar(i)), t), __assign(found, unit(true)), __assign(i, readVar(i).$plus(unit(1)))));
//           self.tmpCount_$eq(__ifThenElse(infix_$eq$eq(readVar(i), self.tmpBuffer.size), unit(-1), readVar(i)));
...
//         val res: this.Rep[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[A, B]] = RecordOps[A](self.tmpBuffer.apply(self.tmpCount)).concatenateDynamic[B](self.tmpLine, self.leftAlias, self.rightAlias);
//         self.tmpCount_$eq(self.tmpCount.$plus(unit(1)));
//         res
//       }, self.NullDynamicRecord[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[A, B]])
//     }
//   }
//   override def hashJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[HashJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = {
//     unit(())
//   }
...
//   type WindowOp[A, B, C] = ch.epfl.data.dblab.legobase.queryengine.volcano.WindowOp[A, B, C]
...
//   type LeftHashSemiJoinOp[A, B, C] = ch.epfl.data.dblab.legobase.queryengine.volcano.LeftHashSemiJoinOp[A, B, C]
...
//         var i: this.Var[Int] = __newVar(unit(0));
//         var found: this.Var[Boolean] = __newVar(unit(false));
//         __whileDo(readVar(found).unary_$bang.$amp$amp(readVar(i).$less(tmpBuffer.size)), __ifThenElse(__app(self.joinCond).apply(t, tmpBuffer.apply(readVar(i))), __assign(found, unit(true)), __assign(i, readVar(i).$plus(unit(1)))));
//         infix_$bang$eq(readVar(i), tmpBuffer.size)
//       }, unit(false))
//     })))
//   }
...
//   case class NestedLoopsJoinOpForeach[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[Unit](Some(self), "foreach", List(List(f))) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOpFindFirst[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "findFirst", List(List(cond))) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOpNullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, D](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeD: TypeRep[D], val di: DummyImplicit) extends FunctionDef[D](Some(self), "NullDynamicRecord", List()) {
//     override def curriedConstructor = (copy[A, B, D] _)
//   }
...
//   case class NestedLoopsJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "next", List(List())) {
//     override def curriedConstructor = (copy[A, B] _)
//   }

//   case class NestedLoopsJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
//     override def curriedConstructor = (copy[A, B] _)
//   }
...
//   case class NestedLoopsJoinOp_Field_Cnt_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[Int])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldSetter[Int](self, "cnt", x$1) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOp_Field_Cnt[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldGetter[Int](self, "cnt") {
//     override def curriedConstructor = (copy[A, B] _)
//   }

//   case class NestedLoopsJoinOp_Field_RightTuple_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[B])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldSetter[B](self, "rightTuple", x$1) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }

//   case class NestedLoopsJoinOp_Field_RightTuple[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldGetter[B](self, "rightTuple") {
//     override def curriedConstructor = (copy[A, B] _)
//   }

//   case class NestedLoopsJoinOp_Field_LeftTuple_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[A])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldSetter[A](self, "leftTuple", x$1) {
//     override def curriedConstructor = (copy[A, B] _).curried
//   }
...
//   case class NestedLoopsJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B]) extends FieldDef[DynamicCompositeRecord[A, B]](self, "NullDynamicRecord") {
//     override def curriedConstructor = (copy[A, B] _)
//     override def isPure = true
...
//   def nestedLoopsJoinOpForeach[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOpForeach[A, B](self, f)
//   def nestedLoopsJoinOpFindFirst[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = NestedLoopsJoinOpFindFirst[A, B](self, cond)
//   def nestedLoopsJoinOpNullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, D](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeD: TypeRep[D], di: DummyImplicit): Rep[D] = NestedLoopsJoinOpNullDynamicRecord[A, B, D](self)
...
//   def nestedLoopsJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = NestedLoopsJoinOpNext[A, B](self)
//   def nestedLoopsJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOpClose[A, B](self)
...
//   def nestedLoopsJoinOp_Field_Cnt_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[Int])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOp_Field_Cnt_$eq[A, B](self, x$1)
//   def nestedLoopsJoinOp_Field_Cnt[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Int] = NestedLoopsJoinOp_Field_Cnt[A, B](self)
//   def nestedLoopsJoinOp_Field_RightTuple_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[B])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOp_Field_RightTuple_$eq[A, B](self, x$1)
//   def nestedLoopsJoinOp_Field_RightTuple[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[B] = NestedLoopsJoinOp_Field_RightTuple[A, B](self)
//   def nestedLoopsJoinOp_Field_LeftTuple_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]], x$1: Rep[A])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = NestedLoopsJoinOp_Field_LeftTuple_$eq[A, B](self, x$1)
...
//   def nestedLoopsJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = NestedLoopsJoinOp_Field_NullDynamicRecord[A, B](self)
//   type NestedLoopsJoinOp[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record] = ch.epfl.data.dblab.legobase.queryengine.volcano.NestedLoopsJoinOp[A, B]
...
//   }
//   implicit def typeNestedLoopsJoinOp[A <: ch.epfl.data.sc.pardis.shallow.Record: TypeRep, B <: ch.epfl.data.sc.pardis.shallow.Record: TypeRep] = NestedLoopsJoinOpType(implicitly[TypeRep[A]], implicitly[TypeRep[B]])
// }
...
//     }
//   }
//   override def nestedLoopsJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[DynamicCompositeRecord[A, B]] = {
//     {
//       var exit: this.Var[Boolean] = __newVar(unit(false));
...
//     }
//   }
//   override def nestedLoopsJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record](self: Rep[NestedLoopsJoinOp[A, B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B]): Rep[Unit] = {
//     unit(())
//   }
...
//   type SubquerySingleResult[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.SubquerySingleResult[A]
...
//   type HashJoinAnti[A, B, C] = ch.epfl.data.dblab.legobase.queryengine.volcano.HashJoinAnti[A, B, C]
...
//   type ViewOp[A] = ch.epfl.data.dblab.legobase.queryengine.volcano.ViewOp[A]
...
//   implicit class LeftOuterJoinOpRep[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]) {
...
//   case class LeftOuterJoinOpForeach[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], f: Rep[((DynamicCompositeRecord[A, B]) => Unit)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val evidence$1: Manifest[B]) extends FunctionDef[Unit](Some(self), "foreach", List(List(f))) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOpFindFirst[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val evidence$1: Manifest[B]) extends FunctionDef[DynamicCompositeRecord[A, B]](Some(self), "findFirst", List(List(cond))) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOpNullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C, D](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val typeD: TypeRep[D], val di: DummyImplicit, val evidence$1: Manifest[B]) extends FunctionDef[D](Some(self), "NullDynamicRecord", List()) {
//     override def curriedConstructor = (copy[A, B, C, D] _)
//   }

//   case class LeftOuterJoinOpOpen[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val evidence$1: Manifest[B]) extends FunctionDef[Unit](Some(self), "open", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class LeftOuterJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val evidence$1: Manifest[B]) extends FunctionDef[Unit](Some(self), "close", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class LeftOuterJoinOpReset[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C], val evidence$1: Manifest[B]) extends FunctionDef[Unit](Some(self), "reset", List(List())) {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }
...
//   case class LeftOuterJoinOp_Field_Hm[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[HashMap[C, ArrayBuffer[B]]](self, "hm") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true

//   }

//   case class LeftOuterJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[A])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[A](self, "tmpLine", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOp_Field_TmpLine[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[A](self, "tmpLine") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class LeftOuterJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[B]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[ArrayBuffer[B]](self, "tmpBuffer", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOp_Field_TmpBuffer[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[ArrayBuffer[B]](self, "tmpBuffer") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class LeftOuterJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[Int])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldSetter[Int](self, "tmpCount", x$1) {
//     override def curriedConstructor = (copy[A, B, C] _).curried
//   }

//   case class LeftOuterJoinOp_Field_TmpCount[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldGetter[Int](self, "tmpCount") {
//     override def curriedConstructor = (copy[A, B, C] _)
//   }

//   case class LeftOuterJoinOp_Field_Evidence$1[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[Manifest[B]](self, "evidence$1") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true
...
//   case class LeftOuterJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit val typeA: TypeRep[A], val typeB: TypeRep[B], val typeC: TypeRep[C]) extends FieldDef[DynamicCompositeRecord[A, B]](self, "NullDynamicRecord") {
//     override def curriedConstructor = (copy[A, B, C] _)
//     override def isPure = true
...
//   def leftOuterJoinOpFindFirst[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], cond: Rep[((DynamicCompositeRecord[A, B]) => Boolean)])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[DynamicCompositeRecord[A, B]] = LeftOuterJoinOpFindFirst[A, B, C](self, cond)
//   def leftOuterJoinOpNullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C, D](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], typeD: TypeRep[D], di: DummyImplicit, evidence$1: Manifest[B]): Rep[D] = LeftOuterJoinOpNullDynamicRecord[A, B, C, D](self)
//   def leftOuterJoinOpOpen[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = LeftOuterJoinOpOpen[A, B, C](self)
//   def leftOuterJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[DynamicCompositeRecord[A, B]] = LeftOuterJoinOpNext[A, B, C](self)
//   def leftOuterJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = LeftOuterJoinOpClose[A, B, C](self)
//   def leftOuterJoinOpReset[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = LeftOuterJoinOpReset[A, B, C](self)
...
//   def leftOuterJoinOp_Field_Hm[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[HashMap[C, ArrayBuffer[B]]] = LeftOuterJoinOp_Field_Hm[A, B, C](self)
//   def leftOuterJoinOp_Field_TmpLine_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[A])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = LeftOuterJoinOp_Field_TmpLine_$eq[A, B, C](self, x$1)
//   def leftOuterJoinOp_Field_TmpLine[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[A] = LeftOuterJoinOp_Field_TmpLine[A, B, C](self)
//   def leftOuterJoinOp_Field_TmpBuffer_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[ArrayBuffer[B]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = LeftOuterJoinOp_Field_TmpBuffer_$eq[A, B, C](self, x$1)
//   def leftOuterJoinOp_Field_TmpBuffer[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[ArrayBuffer[B]] = LeftOuterJoinOp_Field_TmpBuffer[A, B, C](self)
//   def leftOuterJoinOp_Field_TmpCount_$eq[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]], x$1: Rep[Int])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Unit] = LeftOuterJoinOp_Field_TmpCount_$eq[A, B, C](self, x$1)
//   def leftOuterJoinOp_Field_TmpCount[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Int] = LeftOuterJoinOp_Field_TmpCount[A, B, C](self)
//   def leftOuterJoinOp_Field_Evidence$1[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[Manifest[B]] = LeftOuterJoinOp_Field_Evidence$1[A, B, C](self)
...
//   def leftOuterJoinOp_Field_NullDynamicRecord[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C]): Rep[DynamicCompositeRecord[A, B]] = LeftOuterJoinOp_Field_NullDynamicRecord[A, B, C](self)
//   type LeftOuterJoinOp[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C] = ch.epfl.data.dblab.legobase.queryengine.volcano.LeftOuterJoinOp[A, B, C]
...
//   }
//   implicit def typeLeftOuterJoinOp[A <: ch.epfl.data.sc.pardis.shallow.Record: TypeRep, B <: ch.epfl.data.sc.pardis.shallow.Record: TypeRep, C: TypeRep] = LeftOuterJoinOpType(implicitly[TypeRep[A]], implicitly[TypeRep[B]], implicitly[TypeRep[C]])
// }
// trait LeftOuterJoinOpImplicits { this: LeftOuterJoinOpComponent =>
//   // Add implicit conversions here!
// }
// trait LeftOuterJoinOpImplementations { self: DeepDSL =>
//   override def leftOuterJoinOpOpen[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = {
//     {
//       self.leftParent.open();
//       self.rightParent.open();
//       self.rightParent.foreach(__lambda(((t: this.Rep[B]) => {
...
//     }
//   }
//   override def leftOuterJoinOpNext[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[DynamicCompositeRecord[A, B]] = {
//     {
//       var res: this.Var[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[A, B]] = __newVar(self.NullDynamicRecord[ch.epfl.data.sc.pardis.shallow.DynamicCompositeRecord[A, B]]);
//       __ifThenElse(infix_$bang$eq(self.tmpCount, unit(-1)), {
...
//     }
//   }
//   override def leftOuterJoinOpClose[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = {
//     unit(())
//   }
//   override def leftOuterJoinOpReset[A <: ch.epfl.data.sc.pardis.shallow.Record, B <: ch.epfl.data.sc.pardis.shallow.Record, C](self: Rep[LeftOuterJoinOp[A, B, C]])(implicit typeA: TypeRep[A], typeB: TypeRep[B], typeC: TypeRep[C], evidence$1: Manifest[B]): Rep[Unit] = {
//     {
//       self.rightParent.reset();
//       self.leftParent.reset();
//       self.hm.clear();
